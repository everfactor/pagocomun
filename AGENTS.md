...
*   **Partials & Reusability:** Extract common UI components (like cards, tables, indicators) into partials in `app/views/shared/`. For example:
    *   `shared/economic_indicators` for displaying UF and IPC values.
    *   `shared/bills_table` for displaying a standardized table of bills across different namespaces.
